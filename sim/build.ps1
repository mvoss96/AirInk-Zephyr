...
# AIRINK_VERSION has to reach the compiler as a string LITERAL -- src/version.hpp pastes it
# ("v" AIRINK_VERSION). Handing that to gcc as a -D means getting quotes through PowerShell's
# native-argument mangling, which cost this splash its dignity once already: one escape too many and
# the macro became "\"0.1.0\"", so the panel read v"0.1.0" -- and nothing complained, because it
# compiled perfectly. It only showed up in the rendered image.
#
# So: no quoting contest. Write the define into a header and force-include it. What the compiler sees
# is now plain text in a file, which is exactly as readable as it claims to be.
$verHeader = Join-Path $obj "airink_version_sim.h"
Set-Content -Path $verHeader -Encoding ASCII -Value @(
    "/* Generated by sim/build.ps1 from <repo>/VERSION. Do not edit. */",
    "#pragma once",
    "#define AIRINK_VERSION `"$version`""
)
$def = @("-DLV_CONF_INCLUDE_SIMPLE", "-include", $verHeader)
