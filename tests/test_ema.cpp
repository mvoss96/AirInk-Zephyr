...
TEST_CASE("Ema: adopts the first sample instead of averaging up from zero")
{
	Ema<5, 3> gauge{};
	CHECK(gauge.update(3800) == 3800);

	Ema<2> temp{};
	CHECK(temp.update(-1234) == -1234); /* seeds fine below zero */
}

TEST_CASE("Ema: reaches a constant input exactly, never stalling short of it")
{
	/* The old (value*3 + raw)/4 form stalled at 99 while fed a constant 100 forever. */
	CHECK(settle<Ema<2>>(99, 100) > 0);
	CHECK(settle<Ema<2>>(101, 100) > 0);
	CHECK(settle<Ema<5, 3>>(99, 100) > 0);
	CHECK(settle<Ema<5, 3>>(101, 100) > 0);

	/* Negative readings and zero crossings: temperature goes below freezing. */
	CHECK(settle<Ema<2>>(2500, -4500) > 0);
...
TEST_CASE("Ema: a larger shift is a slower filter")
{
	CHECK(settle<Ema<4>>(0, 1000) > settle<Ema<2>>(0, 1000));
}

TEST_CASE("Ema: equal shifts move the same distance either way")
{
	/* To within the one unit that value()'s floor rounding shaves off a downward step. */
...
TEST_CASE("Ema: the battery gauge falls about 4x more readily than it rises")
{
...
TEST_CASE("Ema: approaches a constant monotonically and never overshoots")
{
	Ema<5, 3> e{};
...
TEST_CASE("Ema: the rise/fall asymmetry biases the reading low, but only slightly")
{
	/* That is the price of a pessimistic gauge, and it is paid in the safe direction.
	 * A symmetric filter must not pay it at all. */
	...
	CHECK(asym > -3.0); /* but not wildly so */
}
