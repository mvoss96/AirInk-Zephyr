...
namespace
{
	 ...
	 * happily paints labels over each other, so a row that no longer fits must fail the build, not
	 * the panel (the fifth entry once painted itself through the header rule, silently). */
	...
	/* The Matter mark, rasterised from the official SVG, 22 px (status bar) and 48 px (splash).
	 * Chosen over the Thread logo because that one is hairlines, and a hairline in a 22 px disc is
	 * one pixel of smudge -- these thick arrows survive from 20 px up. Same I1 idiom as the bolt.
	 * NB: a CSA trademark, licensed with certification -- fine for a private build, not a product. */
	 ...
	 * ONE table, every view a row, enforced below: the hand-kept OR-chain it replaces silently
	 * classified two later views as resting, and they flashed black on every step. */
	struct ViewDef
	{
		lv_obj_t **root;
		bool transient;
	...
	/** Hide every content view, so refresh() can un-hide exactly one. Driven off VIEWS: the
	 * hand-kept list it replaces once omitted a new view, which then sat over every screen forever. */
	...
	void build_status_bar(lv_obj_t *scr, bool with_radio)
	{
		...
		// Four ascending bars on a shared baseline; filled = earned, hollow = the level exists but
		// is not reached (what makes "one bar" read as one OF four). The sizes are not free: below
		// 5x6 the smallest hollow bar reads as solid on a 1-bit panel, making "attached, no signal"
		// indistinguishable from "one bar" -- the two states a person placing the device most needs
		// told apart.
	...
	void build_boot(lv_obj_t *scr, const char *build)
	{
		...
		/* The Matter badge, only where it is true -- a build with no radio says so by not saying it,
		 * the same rule as the status bar and the menu row. */
		if (has_radio)
		{
	 ...
	 * per list; show_list() hides the spares), a cursor, a hint. The cursor is created FIRST so the
	 * labels draw on top: white-on-black is the only selection mark B612's ASCII subset allows. */
	...
	/** Build the pairing view: the QR with the manual code always under it (a camera is the thing
	 * most likely to fail, so the fallback is not behind a second screen). lv_qrcode allocates its
	 * I1 draw buffer from the LVGL pool -- the pool's largest single allocation; see log_pool(). */
...
void ui::set_sensor(uint16_t co2_ppm, int32_t temp_x100, uint16_t hum_x100)
{
	...
	// Dedup on the DISPLAYED value -- in the unit on the panel, so a change below what is shown
	// costs no refresh: a stable tick is ~0.16 mAs instead of ~3 mAs (docs/power-analysis.md).
	// Fahrenheit shows whole degrees, coarser than 0.1 C, so the same room refreshes LESS in F.
...
void ui::set_temp_unit(TempUnit u)
{
	...
	// Repaint the number from the kept reading, so the panel never shows 24.3 with an F next to it
	// for the half minute until the next measurement lands.
